...
/*   Updated: 2025/12/05 09:34:30 by achauvie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_img_pl(t_mlx_data *dt, char *txt, int pos_x, int pos_y)
{
	...
	tmp->img = mlx_new_image_from_file((*dt).mlx, txt, &tmp->size, &tmp->size);
	(*dt).player = tmp;
	x_calc = tmp->pos_x * tmp->size;
	y_calc = tmp->pos_y * tmp->size;
	mlx_put_image_to_window((*dt).mlx, (*dt).win, tmp->img, x_calc, y_calc);
}
